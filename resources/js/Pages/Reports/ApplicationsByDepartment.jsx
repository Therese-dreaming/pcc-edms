...
export default function ApplicationsByDepartment({ filters, data }) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Applications by Department</h2>}
        >
            ...
            <div className="py-12">
                <div className="mx-auto max-w-6xl sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                        <h3 className="mb-3 text-sm font-semibold text-gray-700">Total applications by department ({data.grand_total} overall)</h3>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">DPREQ</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">REMIS</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Total</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {data.departments.length === 0 && (
                                    <tr><td colSpan={4} className="px-6 py-4 text-center text-gray-500">No applications for the selected filters.</td></tr>
                                )}
                                    ...
                                    <tr key={row.department}>
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-600">
                                            ...
                                            {Object.entries(row.dpreq_by_status).map(([status, count]) => (
                                                <span key={status} className="ml-2 text-xs text-gray-400">{status}: {count}</span>
                                            ))}
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-600">
                                            {row.remis_total}
