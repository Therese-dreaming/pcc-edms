...
export default function OjtAccommodated({ filters, data }) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">OJTs Accommodated</h2>}
        >
            ...
            <div className="py-12">
                <div className="mx-auto max-w-5xl sm:px-6 lg:px-8">
                    ...
                    <form onSubmit={submit} className="mb-6 flex flex-wrap items-end gap-3 rounded-lg bg-white p-4 shadow-sm">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-600">Granularity</label>
                            <select value={form.granularity} onChange={(e) => setForm({ ...form, granularity: e.target.value })} className="rounded-md border-gray-300 text-sm">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Department/Office</th>
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {data.rows.length === 0 && (
                                    <tr><td colSpan={5} className="px-6 py-4 text-center text-gray-500">No OJT placements for the selected filters.</td></tr>
                                )}
                                {data.rows.map((row, i) => (
                                    <tr key={i}>
