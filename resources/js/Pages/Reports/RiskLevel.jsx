...
export default function RiskLevel({ filters, data }) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Applications by Risk Level</h2>}
        >
            ...
            <div className="py-12">
                <div className="mx-auto max-w-6xl sm:px-6 lg:px-8">
                    ...
                    <form onSubmit={submit} className="mb-6 flex flex-wrap items-end gap-3 rounded-lg bg-white p-4 shadow-sm">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-600">Study Type</label>
                            <select value={form.study_type} onChange={(e) => setForm({ ...form, study_type: e.target.value })} className="rounded-md border-gray-300 text-sm">
                    ...
                    <div className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                        <h3 className="mb-3 text-sm font-semibold text-gray-700">By risk level ({data.total} total)</h3>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Study Type</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Risk Level</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Review Type</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500">Classified</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {data.rows.length === 0 && (
                                    <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-500">No classified applications for the selected filters.</td></tr>
                                )}
                                {data.rows.map((row) => (
